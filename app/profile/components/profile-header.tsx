...
import { Settings, Bell, HelpCircle, Edit, Lock } from "lucide-react"
...
export function ProfileHeader() {
  ...
  const initials = user.firstName && user.lastName
    ...
    : user.username?.[0]?.toUpperCase() || user.primaryEmailAddress?.emailAddress?.[0]?.toUpperCase() || "U"
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="text-xl sm:text-2xl font-bold font-display">{displayName}</h2>
              <p className="text-muted-foreground text-sm sm:text-base">{user.primaryEmailAddress?.emailAddress}</p>
              {user.username && (
                <p className="text-xs sm:text-sm text-muted-foreground">@{user.username}</p>
              )}
              <div className="flex flex-col sm:flex-row gap-2 mt-2">
                <Button size="sm" variant="outline" onClick={() => setIsEditProfileOpen(true)} className="w-full sm:w-auto">
